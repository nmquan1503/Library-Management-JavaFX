...
public class BookView extends ScrollPane implements MainInfo {
  ...
  @FXML
  private HBox ratingCountBox;
  @FXML
  private Label ratingCountTag;
  @FXML
  ...
  /**
   * init image book. if image is null, set default image.
   */
  ...
  /**
   * set stars of book. based on average rating.
   */
  ...
  /**
   * exit book view. close tts. play finish transition.
   */
  @FXML
  ...
  /**
   * call tts api to speak info of book.
   */
  @FXML
  private void Speak() {
    ...
    if (titleLabel != null) {
      if (!titleLabel.getText().isEmpty()) {
        if (BaseController.isTranslate) {
          oup = oup.concat("Book's name: " + titleLabel.getText() + "\n");
        } else {
          oup = oup.concat("Tên sách: " + titleLabel.getText() + "\n");
        }
      }
    }
    if (authorList != null) {
    ...
    if (pageCountLabel != null) {
      if (!pageCountLabel.getText().isEmpty()) {
        ...
        if (BaseController.isTranslate) {
          oup = oup.concat(" pages\n");
        } else {
          oup = oup.concat(" trang\n");
        }
      }
    }
    ...
    if (averageRatingLabel != null) {
      if (!averageRatingLabel.getText().isEmpty()) {
        if (BaseController.isTranslate) {
          ...
          oup = oup.concat("Chất lượng: " + averageRatingLabel.getText() + " trên 5 sao\n");
        }
      }
    }
  ...
  public void setUpLanguage(HashMap<Object, String> viLang, HashMap<Object, String> enLang) {
    if (descriptionLabel != null) {
      if (!descriptionLabel.getText().isEmpty()) {
        ...
        enLang.put(descriptionLabel,
            Translate.translate(descriptionLabel.getText(), Language.VIETNAMESE, Language.ENGLISH));
      }
    }
  }
