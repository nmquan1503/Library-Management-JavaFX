...
public class BooksController extends BaseController{
  @FXML private AnchorPane advertisementPane;
  @FXML private ImageView content1;
  @FXML private ImageView content2;
  @FXML private ImageView content3;
  ...
  @FXML private JFXListView<String> categoriesListView;
  @FXML private TextField titleTextField;
  @FXML private JFXButton removeTextFieldButton;
  @FXML private JFXListView<SuggestionView> titleListView;

  @FXML private TextField pageNumberTextField;
  @FXML private JFXButton nextPageButton;
  @FXML private JFXButton prevPageButton;

  @FXML private JFXListView<SuggestionView> ListBooks;
  ...
  /**
   * Direction of content's movement.
   * Considering of middle content's movement.
   */
  ...
  private Transition advertisementTransition(Direction direction){
    ...
    transition.setOnFinished(e->{
      ...
      if(!advertisementTransitions.isEmpty()){
        ...
        if(nextTransition!=null)nextTransition.play();
      }
    });
    return transition;
  ...
  private void switchToContent2(){
    ...
    else advertisementTransitions.offer(advertisementTransition(Direction.Right));
    if(advertisementTransitions.size()==1){
      Transition nextTransition=advertisementTransitions.peek();
      if(nextTransition!=null)nextTransition.play();
    }
  ...
  private void switchToContent3(){
    ...
    else advertisementTransitions.offer(advertisementTransition(Direction.Left));
    if(advertisementTransitions.size()==1){
      Transition nextTransition=advertisementTransitions.peek();
      if(nextTransition!=null)nextTransition.play();
    }
  ...
  private void setupFocusTextField(){
    ...
    titleTextField.focusedProperty().addListener((observable, oldValue, newValue) ->{
      if(newValue){
        ...
        if(suggestionView!=null)showBook(suggestionView.getID());
  ...
  private void showBook(int idBook){
    Thread thread=new Thread(()->{
      BookView bookView=new BookView(library.getBook(idBook));
      Platform.runLater(()->{
  ...
  private void CreateBookSuggestions(){
    ...
    Thread thread=new Thread(()->{
      ArrayList<Suggestion> listSuggestions=library.getBookSuggestions(prefix);
      Platform.runLater(()->{
  ...
  private void initView(){
    Thread thread=new Thread(()->{
      listSuggestions=library.getBookSuggestions("");
      Platform.runLater(()->{
        setListBooks(1);
  ...
  private void Search(){
    ...
    Thread thread1=new Thread(()->{
      if(!category.isEmpty()) {
        for (int i = 21; i < listSuggestions.size(); i++) {
          Book book = library.getBook(listSuggestions.get(i).getId());
    ...
    Thread thread=new Thread(()->{
      listSuggestions=library.getBookSuggestions(titleTextField.getText());
      if(!category.isEmpty()) {
