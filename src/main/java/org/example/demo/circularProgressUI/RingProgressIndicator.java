...
import javafx.css.converter.SizeConverter; // Correct public converter
  ...
  public RingProgressIndicator() {
    this.getStylesheets()
        .add(RingProgressIndicator.class.getResource(
                "/org/example/demo/circularStyle/ringprogress.css")
            .toExternalForm());
  ...
  private static class StyleableProperties {
    ...
    private static final CssMetaData<RingProgressIndicator, Number> RING_WIDTH =
        new CssMetaData<RingProgressIndicator, Number>(
            "-fx-ring-width",
            StyleConverter.getSizeConverter(), // Use StyleConverter for size conversion
            22) {
